...
// GET /api/invoices/calculate-price - MUST be before /:id to avoid Express matching 'calculate-price' as an id
router.get('/calculate-price', requireAuth, (req, res) => {
  ...
  if (account_customer_id) {
    const acct = db.prepare('SELECT discount_percent FROM account_customers WHERE id = ?').get(account_customer_id);
...
router.get('/next-number', requireAuth, (req, res) => {
  const carparkId = req.session.carparkId || 1;
  const result = db.prepare('SELECT MAX(invoice_number) as max FROM invoices WHERE carpark_id = ?').get(carparkId);
  const nextNum = (result.max || 18999) + 1;
...
router.get('/', requireAuth, (req, res) => {
  ...
  if (date) {
    query += ' AND DATE(i.date_in) = ?';
    params.push(date);
  }
  if (status) {
    query += ' AND i.paid_status = ?';
    params.push(status);
  }
  if (customer_id) {
...
router.post('/', requireAuth, (req, res) => {
  ...
  const {
    ...
    rego, make, first_name, last_name, phone, email,
  ...
  const existing = db.prepare('SELECT id FROM invoices WHERE invoice_number = ? AND carpark_id = ?').get(invoice_number, carparkId);
  if (existing) {
  ...
  // Update key box if key assigned
  if (key_number && !no_key) {
    db.prepare("UPDATE key_box SET status = 'in_use', invoice_id = ? WHERE carpark_id = ? AND key_number = ?")
      .run(result.lastInsertRowid, carparkId, key_number);
  }

  // Update customer credit if used
  if (credit_applied > 0 && customer_id) {
    db.prepare('UPDATE customers SET notes = notes WHERE id = ?').run(customer_id);
  }
...
router.put('/:id', requireAuth, (req, res) => {
  ...
  const carparkId = req.session.carparkId || 1;
  const {
    key_number, no_key, rego, make, first_name, last_name, phone, email,
    date_in, time_in, return_date, return_time, stay_nights,
    flight_info, flight_type, total_price, credit_applied, discount_percent,
    paid_status, payment_amount, payment_method, paid_status_2, payment_amount_2, payment_method_2,
  ...
  const existing = db.prepare('SELECT * FROM invoices WHERE id = ? AND carpark_id = ?').get(id, carparkId);
  ...
  db.prepare(`
    UPDATE invoices SET
      key_number = ?, no_key = ?, rego = ?, make = ?, first_name = ?, last_name = ?,
  ...
  `).run(
    key_number || null, no_key ? 1 : 0, rego, make, first_name, last_name,
    phone, email, date_in, time_in, return_date, return_time,
...
router.post('/:id/void', requireAuth, (req, res) => {
  ...
  db.prepare("UPDATE invoices SET void = 1, picked_up = 'Voided', updated_at = CURRENT_TIMESTAMP WHERE id = ?").run(id);
...
router.post('/:id/refund', requireAuth, (req, res) => {
  ...
  db.prepare("UPDATE invoices SET refund_amount = ?, refund_reason = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?")
...
router.get('/:id/pdf', requireAuth, (req, res) => {
  ...
  doc.text(`Vehicle: ${invoice.rego || ''} ${invoice.make || ''}`);
  ...
  if (invoice.discount_percent > 0) {
    doc.text(`Discount: ${invoice.discount_percent}%`);
  }
  if (invoice.credit_applied > 0) {
    doc.text(`Credit Applied: $${parseFloat(invoice.credit_applied).toFixed(2)}`);
  }
  ...
  if (invoice.payment_amount_2 > 0) {
    doc.text(`2nd Payment: ${invoice.paid_status_2} - $${parseFloat(invoice.payment_amount_2 || 0).toFixed(2)}`);
  }
